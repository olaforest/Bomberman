	...
	public void updateEnemiesPosition() { //this is the method that is called to move everything
		...
		for (Enemy temp : enemies){
			if (!temp.isDead()) {
				 ...
				 * for example, if an enemy spawns with two concrete blocks to the left and right of it
				...
				/* Now we check for random changes in path. If the enemy has an intelligence level of higher than 1, we
				 * check if it is at an intersection. This is done by a combination of seeing if it is in the right
				 * spots with regards to concrete blocks (if you have two on either side or up and down you are
				 * definitely not at an intersection), and then checking if the intersection is blocked by bombs or
				 * bricks using randomChangeCheck.
				 */
